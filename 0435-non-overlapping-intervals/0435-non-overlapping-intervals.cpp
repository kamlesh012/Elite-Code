...
public:
    ...
    sort(a.begin(),a.end(),[&](vector<int> &f,vector<int> &s){
       if(f[1]==s[1])return f[0]<s[0];
        ...
        //suppose next2 also intersects current interval that means it would definitely intersect next as well(because these are sorted in increasing order of end time)(see TC1).
